...
test('calls the onOptionSelected prop with the selected option and its index if passed', () => {
    ...
    const { getAllByRole, getByTestId, debug } = render(<Select options={options} onOptionSelected={onOptionSelected} />)
...
test('the button label changes to the selected option label', () => {
    const { getAllByRole, getByTestId, debug } =render(<Select options={options} />)
